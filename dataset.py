...
def make_dataset(dir, stop=12000):
    images = []
    count = 0
    assert os.path.isdir(dir), '%s is not a valid directory' % dir
    for root, _, fnames in sorted(os.walk(dir)):
        for fname in fnames:
            if is_image_file(fname) and int(fname[:-4]) <= stop:
...
class UnpairedDepthDataset(data.Dataset):
    def __init__(self, root, root2, opt, transforms_r=None, mode='train', midas=False, depthroot=''):
        ...
        if mode == 'train':
            ...
            self.data = self.data[:cutoff]         #input
            self.img2 = self.img2[:cutoff]         #output
    ...
    def __getitem__(self, index):
        ...
        if self.mode != 'train':
            A_transform = self.transform_r          #test   图片预处理
        ...
        if self.mode == 'train':
            index_B = random.randint(0, self.min_length - 1)
            cur_path = self.img2[index_B]
...
class OneDataset(data.Dataset):
    def __init__(self, root, opt, transforms_r=None, mode='train'):
    ...
    def __getitem__(self, index):
        ...
        input_dict = {'r': img_r, 'path': img_path, 'index': index, 'name': base,'depth': img_depth}
